...
Player.prototype.initDownloadWorker = function () {
    var self = this;
    this.downloadWorker = new Worker("static/video/downloader.js");
...
Player.prototype.play = function (target, url, canvas, callback, waitHeaderLength, isStream) {
    this.logger.logInfo("Play " + url + ".");
    this.wasmPlayer = target
...
Player.prototype.onVideoParam = function (v) {
    ...
    // xuan
    this.wasmPlayer.duration = Math.floor(this.duration / 1000)
...
Player.prototype.updateTrackTime = function () {
    if (this.playerState == playerStatePlaying && this.pcmPlayer) {
        ...
        this.wasmPlayer.currentPlayTime = Math.floor(currentPlayTime)
        // console.log(currentPlayTime * 1000, this.duration, '00000000000000')
        if (this.timeTrack) {
            this.timeTrack.value = 1000 * currentPlayTime;
        }

        // 当currentPlayTime === this.duration时候就是播放结束, 告诉wasm-player播放结束
        if(!this.isStream && (1000 * currentPlayTime >= this.duration)) {
            this.wasmPlayer.playEnd()
            this.stopTrackTimer()
        }
