  ...
  <div>
    ...
    >
      ...
      <ul class="info-list" v-show="!isDetail">
        <li><span v-if="maxSize">大小：{{ maxSize }}KB以内</span></li>
        <li><span v-if="mixMsg">{{ mixMsg }}</span></li>
        <li><span v-if="format.length>0">格式：{{ format.join(', ') }}</span></li>
        <li><span v-if="width && height">分辨率：{{ width }}*{{ height }}px</span></li>
        <li><span v-if="width && !height">分辨率：宽 {{ width }}px</span></li>
        <li><span v-if="!width && height">分辨率：高 {{ height }}px</span></li>
        <li><span v-if="otherMsg">{{ otherMsg }}</span></li>
      </ul>
    </div>
  ...
  computed: {
    sheep() {
      ...
      if (this.width && this.height && this.width !== this.height) {
        let name = this.width < this.height ? { min: "width", max: "height" } : { min: "height", max: "width" }
  ...
  methods: {
        ...
        reader.onload = async function (e) {
          ...
          image.onload = function () {
            ...
            if (that.width && that.height) {
              isAllow = width === that.width && height === that.height ? true : false
    ...
    async uploadSuccess(res) {
      if (res.success) {
        ...
        this.$emit("on-change", res.data.picUrl, res.data)
      } else {
