...
import { hexagonEdit } from "_o/api/priceCenter";
  ...
  data() {
    return {
      ...
      validateForm: {
        ...
        typeTimeList: [{ required: true }],
        carLevelList: [{ required: true }],
        effectDate: [
          {
            required: true,
  ...
  methods: {
    ...
    init(val) {
      ...
      this.form = {
        ...
        serviceType: this.getProductName(val.serviceType), // 产品类型
        carLevelList: this.getCarLevelName(val.carLevelList, val.serviceType), // 车级类型
        cityName: val.cityName, // 城市
        typeTimeList: getTypeTimeName(val.typeTimeList), // 订单类型
