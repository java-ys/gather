 ...
 * @LastEditTime: 2022-01-20 20:09:51
 * @Description:
-->
<template>
  <div>
...
export default {
  ...
  setup: (props, { root }) => {
    ...
    // 查询条件
    store.dispatch('getAuthCityList')
    watch(
      () => state.common.authCityList,
      (count) => {
        setInputDropList('cityCodes', count)
      }
    )
    setInputDropList('expandBizLine', states.businessGradeList)
    setInputDropList('businessType', states.bizTypeList)
