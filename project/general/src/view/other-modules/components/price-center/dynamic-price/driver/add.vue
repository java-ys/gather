<template>
  <div>
    <Modal
      ...
      :closable="false"
    >
      <Form ref="_form" :model="addForm" :label-width="150" :rules="formRules">
        <Row>
...
import { driver_special_params } from "./fields";
  ...
  methods: {
    ...
    async save() {
      ...
      const [err, data] = await this.toResult(
        createService({ ...p, ...driver_special_params })
      );
      if (err) {
        return;
      }
    ...
    timeIntersection(date1, date2) {
      if (
        date1.filter(Boolean).length < 2 ||
        date2.filter(Boolean).length < 2
      ) {
        return false;
      }
