...
<script>
import { inputList, extraLine, extraBiz, defaultInner } from "../fields.js";
import { tableColumns, driver_special_params } from "./fields";
...
export default {
  ...
  methods: {
    init() {
      this.expandBizLineList = this.businessGradeList.filter(
        (it) => !extraLine(it.value)
      );
      findO("expandBizLine", "bind_key", this.inputList).dropList =
        this.expandBizLineList;
      ...
      this.$Modal.confirm({
        ...
        onOk: async () => {
          const [err] = await this.toResult(
            changeStatusService({ uuid, status, ...driver_special_params })
          );
          if (err) {
            return;
          }
    ...
    async edit({ uuid }, type = 1) {
      const [err, data] = await this.toResult(
        getDetailService({ uuid, ...driver_special_params })
      );
      if (err) {
        return;
      }
