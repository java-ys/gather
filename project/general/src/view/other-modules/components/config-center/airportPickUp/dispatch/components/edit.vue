 ...
 * @LastEditTime: 2021-11-24 19:21:59
 * @Description: 新增表单弹窗
-->
<template>
  <Form
  ...
  >
    ...
    <div class="form-item flex">
      ...
      <FormItem label="车型等级" prop="carLevel">
        <Select v-model="formData.carLevel">
          <Option
...
<script>
import { updateDispatchStrategy } from "_o/api/airportPickUp.js";
...
export default {
  mixins: [editMixin, crossRangeMixin, commonMixin],
  props: {
    cityList: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    async handleCreate(params) {
      try {
        this.$Spin.show();
        const res = await updateDispatchStrategy(params);
