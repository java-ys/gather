 ...
 * @LastEditTime: 2021-12-02 10:46:31
 * @Description: 新增表单弹窗
-->
<template>
...
import { updatePricingStrategy, queryPricingStrategyDetail } from "_o/api/airportPickUp.js";
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs.priceForm.validate(valid => {
        if (valid) {
          ...
          this.handleUpdate({ ...this.formData, uuid: this.uuid });
        }
      });
    },
    // 编辑保存
    async handleUpdate(params) {
      ...
      try {
        ...
        const res = await updatePricingStrategy(params);
