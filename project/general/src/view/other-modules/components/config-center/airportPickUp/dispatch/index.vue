<!--
 * @Author: Evil Cheng
 * @Date: 2021-11-08 11:10:24
 * @LastEditTime: 2021-11-23 15:04:55
 * @Description:派单策略
-->
<!--
 * 每个状态对应的操作按钮
...
<template>
  <div class="container">
    ...
    <div class="add-reassign-strategy">
      <Button type="primary" @click="handleAdd">新增派单策略</Button>
    </div>
    ...
    <Modal v-model="addModel" title="新增派单策略" width="800" footer-hide>
      <div>
        <addForm :cityList="cityList" @close="addModel = false" @update="refreshTable" />
      </div>
    </Modal>
    <Modal v-model="detailModel" title="派单策略详情" width="800" footer-hide @on-visible-change="handleDetailVisible">
      <div>
        <detailForm v-if="detailModel" :uuid="uuid" :cityList="cityList" @close="detailModel = false" />
      </div>
    </Modal>
    <Modal v-model="editModel" title="编辑派单策略" width="800" footer-hide @on-visible-change="handleDetailVisible">
      <div>
        <editForm v-if="editModel" :uuid="uuid" :cityList="cityList" @close="editModel = false" @update="refreshTable" />
      </div>
    </Modal>
    <Modal v-model="copyModel" title="复制派单策略" width="800" footer-hide @on-visible-change="handleDetailVisible">
      <div>
...
import { queryDispatchStrategy, updateDispatchStrategyStatus } from "_o/api/airportPickUp.js"
import { filterOrderType, filterOrderStatus } from "../nums"
export default {
      ...
      columns: [
...

        {
          ...
          render: (h, params) => {
            ...
            return h("div", businessKeyValue[params.row.extendBizType]);
          }
        },
        {
          title: "状态",
  ...
  methods: {
    ...
    async getTableList() {
      ...
      this.loading = true
      try {
        const res = await queryDispatchStrategy(params)
        this.loading = false
    ...
    async changeStatus(item) {
      ...
      const res = await updateDispatchStrategyStatus(params)
      const { success, data, msg } = res.data
      if (success) {
        // eslint-disable-next-line require-atomic-updates
        item.status = data // 更新列表状态
