 ...
 * @LastEditTime: 2021-11-18 17:11:59
 * @Description: 新增表单弹窗
-->
<template>
  <Form ref="dispatchForm" :model="formData" :rules="formRules" :label-width="120">
      ...
      >
        <div class="playlist-content">
          <div v-if="index === 0" class="form-item flex">
            <FormItem label="时间范围" :prop="'highDispatch.' + index + '.minHour'"
                      :rules="{ type: 'number',required: true, message: '时间范围不能为空', trigger: 'blur' }"
            >
              可调度时间大于等于
          ...
          <div v-else class="form-item" style="display: inline-block;">
            <FormItem label="时间范围" class="connect-item" :prop="'highDispatch.' + index + '.minHour'"
                      :rules="{ type: 'number',required: true,validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              可调度时间大于等于
              <InputNumber v-model="item.minHour" :min="0"></InputNumber>
              小时，
            </FormItem>
            <FormItem :label-width="0" class="connect-item" :prop="'highDispatch.' + index + '.maxHour'"
                      :rules="{ type: 'number',required: true,validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              小于
              <InputNumber
          ...
          <div class="form-item flex">
            <FormItem label="司机报价系数" :prop="'highDispatch.' + index + '.driverRate'"
                      :rules="validNumber"
            >
              <InputNumber
      ...
      >
        <div class="playlist-content">
          <div class="form-item" style="display: inline-block;">
            <FormItem label="时间范围" class="connect-item" :prop="'wholeCityDispatch.' + index + '.minHour'"
                      :rules="{ type: 'number',required: true,validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              可调度时间大于等于
              <InputNumber v-model="item.minHour" :min="0"></InputNumber>
            ...
            <FormItem :label-width="0" class="connect-item" :prop="'wholeCityDispatch.' + index + '.maxHour'"
                      :rules="{ type: 'number',required: true,validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              小于
              <InputNumber
          ...
          <div class="form-item flex">
            <FormItem label="司机报价系数" :prop="'wholeCityDispatch.' + index + '.driverRate'"
                      :rules="validNumber"
            >
              <InputNumber
                v-model="item.driverRate"
    ...
    <div class="playlist">
      ...
      >
        <div class="playlist-content">
          <div class="form-item" style="display: inline-block;">
            <FormItem label="时间范围" class="connect-item" :prop="'directDispatch.' + index + '.minHour'"
                      :rules="{ type: 'number',required: true, validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              可调度时间大于等于
              <InputNumber v-model="item.minHour" :min="0"></InputNumber>
            ...
            <FormItem :label-width="0" class="connect-item" :prop="'directDispatch.' + index + '.maxHour'"
                      :rules="{ type: 'number',required: true, validator: timeRangeValidate(item), trigger: 'blur' }"
            >
              小于
              <InputNumber
          ...
          <div class="form-item flex">
            <FormItem label="司机报价系数" :prop="'directDispatch.' + index + '.driverRate'"
                      :rules="validNumber"
            >
              <InputNumber
                v-model="item.driverRate"
    ...
    <div class="playlist">
      ...
      >
        <div class="playlist-content">
          ...
          <div class="form-item flex">
            <FormItem label="司机报价系数" :prop="'workDispatch.' + index + '.driverRate'"
                      :rules="validNumber"
            >
...
export default {
  mixins: [crossRangeMixin, commonMixin],
  props: {
    cityList: {
      type: Array,
      default: () => []
