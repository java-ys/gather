 ...
 * @LastEditTime: 2021-12-02 10:19:09
 * @Description: 新增表单弹窗
-->
<template>
  <Form ref="priceCopyForm" :model="formData" :label-width="120" :rules="formRules">
...
import { createPricingStrategy, queryPricingStrategyDetail } from "_o/api/airportPickUp.js";
import { ORDER_TYPES, CALCULATE_NODE } from "../../nums";
import { effectiveTime } from "../../validate";
import privateCommonMixin from "./common.mixins"
export default {
  mixins: [privateCommonMixin],
  props: {
    cityList: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    async getDetail(uuid) {
      ...
      if (success) {
        ...
        // 获取产品线以及车型等级
        this.$nextTick(() => {
          const business = this.businessList.find(v => v.value === data.expandBizLine);
          this.productList = business.children;
          const product = this.productList.find(v => v.bizType === data.businessType);
    ...
    handleSubmit() {
      this.$refs.priceCopyForm.validate(valid => {
        if (valid) {
          // 判断时段是否满足完整的时间段
    ...
    async handleCreate(params) {
      ...
      try {
        ...
        if (success) {
          ...
          this.$refs.priceCopyForm.resetFields();
        } else {
          this.$Message.error(msg);
        }
      } catch (error) {
