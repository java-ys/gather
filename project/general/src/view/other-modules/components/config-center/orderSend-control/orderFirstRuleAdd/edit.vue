 ...
 * @LastEditTime: 2021-11-08 17:15:00
 * @Description:
-->
<template>
  <div>
...
import { updataStrategy, getInfoStrategy } from "_o/api/configData.js";
export default {
  components: {
    basicForm,
    valuePriority,
  ...
  data() {
    return {
      disabled: true, // 编辑情况部分表单数据disable状态
      prioritys: [],
      basicFormData: {},
      valuePriorityData: {
  ...
  methods: {
    ...
    async saveData() {
      ...
      const params = {
        ...
        ...valueData,
        ...sourceData,
        GroupMemberSaveOrUpdateReq: relativeData // key值保存跟编辑不同
      };
      const res = await updataStrategy(params);
      if (res.data.success) {
