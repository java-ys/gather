...
let carType = {
  ...
  minWidth: 100,
  render: (h, params) => {
    return h("div", carTypeMap[params.row.carLevel] || "")
  }
}
let typeTime = {
  title: "订单类型",
...
export const bookingColumns = (that) => {
  return {
    ...
    businessType: {
      ...
      minWidth: 100,
      render: (h, params) => {
        return h("div", bookingLineMap[params.row.businessType] || "")
      }
    },
    reassignTimeLimit: {
      title: "改派时间限制(分钟)",
...
export let productLine = {
  name: "cascader-input", // 文本输入框名
  bind_key: ["businessType", "carLevel"], // 返回数据的key名
