<template>
  <div>
    ...
    <div v-if="!isChild" style="margin-top: 20px">
      <Form ref="formInline" :model="infoData" :label-width="100" inline>
        <FormItem label="奖励名称" prop="rewardName">
          ...
          <Button type="primary" class="margin15" @click="searchData(infoData)">查询</Button>
    ...
    <div class="footer">
      <Button v-if="Object.keys(fatherData).length>0" @click="step">{{isChild?'上一步':'下一步'}}</Button>
    </div>
  </div>
</template>
<script>
  ...
  export default {
        props:{
          ...
          limitType:{
              type:Array,
              default:()=>{
                  return ["score", "cash", "giftCard", "giveCoin", "charSub", "opeCharge", "othEleComp"] // 只能选择优惠券和第三方api发券
              }
          },
          time:{
              type:Array,
      ...
      methods:{
          ...
          selectReward(row){
              ...
              if(row.rewardPattern===1||row.rewardPattern===3){
                  ...
                  if(this.selectedRewardEventTypeCode>=38&&this.selectedRewardEventTypeCode<=41){
              ...
              */
              ...
              if(canSelect){
                  let obj = {
                    rewardId: this.fatherData.uuid,
                    begin: this.fatherData.beginTime,
                    end: this.fatherData.endTime,
                    rewardName: this.fatherData.rewardName,
                    subRewardId: row.uuid,
                    subRewardName: row.subRewardName
                  }
                  console.log(obj);
                  this.$emit('close',obj);
              }else {
