 ...
 * @LastEditTime: 2021-10-28 22:53:13
 -->
<template>
  <div class="update-package-price">
    <Form ref="formValidate" :model="params" :rules="ruleValidate" :label-width="180">
      ...
      <div class="flex-box between">
        <FormItem label="套餐名称 :" prop="wrapName">
          ...
          <Input v-if="addViewStatus" v-model="params.wrapName" :maxlength="30" placeholder="请输入套餐名称"
                 style="width: 200px"
          />
        </FormItem>
      </div>
      ...
      <div class="flex-box between">
        <FormItem v-if="params.wrapType === 0" label="套餐里程（公里）:" prop="wrapDistance">
          <span v-if="!addViewStatus" class="width100">{{params.wrapDistance}}</span>
          <InputNumber v-if="addViewStatus" v-model="params.wrapDistance" style="width: 200px" :min="0.01" :max="999.99"
                       :step="0.01"
                       placeholder="请输入"
                       @on-blur="FormatNumber(params.wrapDistance,'params','wrapDistance')"
          ></InputNumber>
        </FormItem>
      </div>
      <div class="flex-box between">
        <FormItem label="套餐时长（分钟）:" prop="wrapTime">
          ...
          <InputNumber v-if="addViewStatus" v-model="params.wrapTime" style="width: 200px" :min="1" :max="1440"
                       :precision="0" placeholder="请输入"
          ></InputNumber>
        </FormItem>
      </div>
      <div v-if="params.wrapType === 0" class="flex-box between">
        <FormItem v-if="addViewStatus" label="套餐信用金比例（%）:" prop="wrapMarginRatio">
          <InputNumber v-model="params.wrapMarginRatio" style="width: 200px" :min="1" :max="1000" :step="1"
                       :precision="0" placeholder="请输入"
          ></InputNumber>
        </FormItem>
        <FormItem v-if="!addViewStatus && params.wrapMarginRatio" label="套餐信用金比例（%）:">
          <span class="width100">{{params.wrapMarginRatio}}</span>
        </FormItem>
      </div>
      <Form v-for="(item,index) in params.packagePriceList" :ref="'gradeValidate' + index" :key="index - 100" class="rel-box dot-box"
            :model="item" :rules="ruleValidate" :label-width="80"
      >
        <div class="flex-box" style="margin-bottom: 24px;">
          <FormItem label="车型等级:" prop="carLevel" style="margin-bottom: 0">
            <Select v-if="addViewStatus"
                    ...
                    style="width: 200px"
            >
              <Option v-for="(v,index) in carLevelList" :key="index" :value="v.vehicleLevel" :disabled="v.disabled">
                {{v.vehicleName}}
              </Option>
            </Select>
          </FormItem>
          ...
          <Icon v-if="index > 0 && addViewStatus" class="remove-icon" type="ios-remove-circle-outline"
                @click="removeTrip(item.carLevel,index)"
          />
        </div>
      </Form>
    ...
    <Modal
      ...
      :width="950"
      :styles="{ top: '20px' }"
      :title="carTypeMapAll['8#' + nowCarLeval]"
      :closable="false"
      :mask-closable="false"
    >
      <a-calendar v-if="hackReset" style="margin: 10px;border-bottom: 1px dashed #999" :validRange="timeRange()">
        ...
        >
          <b v-if="addViewStatus" class="price">￥{{getDatePrice(dateValue)}}</b>
          <b v-else class="price">￥{{detailPriceList.includes(formatDate(dateValue._d)) ? getDeatilPrice(dateValue) :
            getDatePrice(dateValue)}}</b>
        </div>
      </a-calendar>
      <Form ref="dateValidate" v-model="priceForm" class="rel-box dot-box" :rules="ruleValidate">
        <div v-if="addViewStatus">
          <div v-if="selectedList.length">
            <!-- row2 -->
            <div class="flex-box">
              <FormItem label="套餐定价（元）：" prop="packagePrice" class="list-label">
                <InputNumber v-model="priceForm.packagePrice" style="width: 120px" :min="1" :max="99999" :step="1"
                             :precision="0" placeholder="请输入套餐定价"
                             @on-blur="FormatNumber(priceForm.packagePrice,'priceForm','packagePrice',0)"
                ></InputNumber>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="里程费（元/公里）：" prop="mileageCost" class="list-label">
                <InputNumber v-model="priceForm.mileageCost" style="width: 120px" :min="0.01" :max="99.99" :step="0.01"
                             placeholder="请输入里程费"
                             @on-blur="FormatNumber(priceForm.mileageCost,'priceForm','mileageCost')"
                ></InputNumber>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="时长费（元/分钟）：" prop="timeCost" class="list-label">
                <InputNumber v-model="priceForm.timeCost" style="width: 120px" :min="0.01" :max="99.99" :step="0.01"
                             placeholder="请输入时长费"
                             @on-blur="FormatNumber(priceForm.timeCost,'priceForm','timeCost')"
                ></InputNumber>
              </FormItem>
            </div>
            <!-- row3 -->
            <div class="flex-box">
              <FormItem v-if="params.wrapType === 0" label="远途里程费（元/公里）：" prop="haulBackTripFare" class="list-label">
                <InputNumber v-model="priceForm.haulBackTripFare" style="width: 120px" :min="0" :max="99.99"
                             :step="0.01"
                             placeholder="请输入远途里程费"
                             @on-blur="FormatNumber(priceForm.haulBackTripFare,'priceForm','haulBackTripFare')"
                ></InputNumber>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="远途里程（公里）：" prop="haulBackTrip" class="list-label">
                <InputNumber v-model="priceForm.haulBackTrip" style="width: 120px" :min="0" :max="999"
                             :step="1" placeholder="请输入远途里程"
                             @on-blur="FormatNumber(priceForm.haulBackTrip,'priceForm','haulBackTrip',0)"
                ></InputNumber>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="回程费（元/公里）：" prop="returnFee" class="list-label">
                <InputNumber v-model="priceForm.returnFee" style="width: 120px" :min="0" :max="99.99" :step="0.01"
                             placeholder="请输入回程费"
                             @on-blur="FormatNumber(priceForm.returnFee,'priceForm','returnFee',2)"
                ></InputNumber>
              </FormItem>
            </div>
            <!-- row4 -->
            <div class="flex-box">
              <FormItem v-if="params.wrapType === 0" label="免费等待时长（分钟）" prop="freeWaitTime" class="list-label">
                <InputNumber v-model="priceForm.freeWaitTime" style="width: 120px" :min="0" :max="999"
                             :step="1" placeholder="请输入免费等待时长"
                             @on-blur="FormatNumber(priceForm.freeWaitTime,'priceForm','freeWaitTime',0)"
                ></InputNumber>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="超时等待费（元/分钟）：" prop="beyondWaitFare" class="list-label">
                <InputNumber v-model="priceForm.beyondWaitFare" style="width: 120px" :min="0" :max="99.99" :step="0.01"
                             placeholder="请输入超时等待费"
                             @on-blur="FormatNumber(priceForm.beyondWaitFare,'priceForm','beyondWaitFare')"
                ></InputNumber>
              </FormItem>
            </div>
          </div>
        ...
        <div v-else>
          <div v-if="detailForm.packagePrice">
            ...
            <div class="price-detail-box">
              ...
              <FormItem v-if="params.wrapType === 0 && detailForm.mileageCost" label="里程费（元/公里）：" class="list-label">
                <span>{{detailForm.mileageCost}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0 && detailForm.timeCost" label="时长费（元/分钟）：" class="list-label">
                <span>{{detailForm.timeCost}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="远途里程费（元/公里）：" class="list-label">
                <span>{{detailForm.haulBackTripFare}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="远途里程（公里）：" class="list-label">
                <span>{{detailForm.haulBackTrip}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="回程费（元/公里）：" class="list-label">
                <span>{{detailForm.returnFee || '-'}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="免费等待时长（分钟）：" class="list-label">
                <span>{{detailForm.freeWaitTime}}</span>
              </FormItem>
              <FormItem v-if="params.wrapType === 0" label="超时等待费（元/分钟）：" class="list-label">
      ...
      <div class="popView-footer-wrap">
        <Button size="large" type="text" style="margin-right: 8px;" @click="saveDatePrice(0)">取消</Button>
...
import { packageTypeMap, carTypeMapAll, getCarLevelList } from "_g/config/status-map";
...
export default {
  ...
  mounted() {
    this.getDefaultDate()
    // 处理所有车型等级
    const childrens = this.bizTypeList.map(v => {
      return v.children
    }).flat()
    for (let item of childrens) {
      const exit = this.carLevelList.find(v => v.vehicleLevel === item.vehicleLevel)
      if (!exit) {
        this.carLevelList.push(item)
      }
    }
  },
  methods: {
    ...
    timeRange() {
      ...
      let minMonth = null; let maxMonth = null; let minYear = null; let
        maxYear = null
      if (curMonth <= 6) {
        minYear = curYear - 1
        minMonth = 5 + curMonth
    ...
    // 浮点数相乘
    accMul(arg1, arg2) {
      let m = 0; let s1 = arg1.toString(); let
        s2 = arg2.toString();
      try {
        m += s1.split(".")[1].length;
      } catch (e) {
    ...
    /* Form ---------------------------------E */
    getCarLevelName(val) {
      const { carLevelKeyValue } = this.businessGradeName
      return carLevelKeyValue[val] || "-"
    },
  }
};
</script>
