      ...
      if (this.value) {
        if (this.parentParams.detailUuid === null || this.type === "add") {
          ...
          if (this.parentParams.relationStrategyUuid) {
            this.parentParams.detailUuid = this.parentParams.relationStrategyUuid
            this.getDetail();
          } else {
  ...
  methods: {
    ...
    getDetail() {
      ...
      priceStrategyDetail({ uuid: this.parentParams.detailUuid }).then((res) => {
        ...
        params = {
          ...
          relationStrategyUuid: this.parentParams.relationStrategyUuid,
        };
        ...
        if ((this.type === "copy" && this.params.relationStrategyUuid) || this.type === "add") {
    ...
    validatePassenger() {
      ...
      if (
        ...
        let workRestDayList = this.params.workRestDayList ? (this.params.workRestDayList.length == 1 ? this.params.workRestDayList[0] : 1) : null;
        // 获取当前生效中的乘客定价规则
        getPriceStrategyList({
          ...
          if (res.data.success) {
            ...
            this.params.passengerViewStatus = true;
            if (item) {
              if (item.startTrip !== null && item.startDuration !== null) {
                this.params.startTrip = item.startTrip || 0;
                this.params.startDuration = item.startDuration || 0;
              ...
              // this.params.workRestDayList = item.workRestDayType ? [item.workRestDayType] : [1, 2];
              if (!this.params.relationStrategyUuid || item.uuid !== this.params.relationStrategyUuid) {
    ...
    // },
    handleChoseArea(code) {
      this.setAreaName(code);
      this.validatePassenger();
    },
    setAreaName(code) {
      if (code) {
    ...
    commitBefore(temp) {
      ...
      if (
        ...
        if (!this.params.relationStrategyUuid) {
          this.$Message.error("请先配置乘客定价策略");
          return false;
        }
        return this.commit(temp);
        ...
        return this.$refs.formValidate
          ...
          .then(() => {
            if (isSuccess1 && isSuccess2) {
              ...
              params.relationStrategyUuid = this.params.relationStrategyUuid;
              // 夜间费
