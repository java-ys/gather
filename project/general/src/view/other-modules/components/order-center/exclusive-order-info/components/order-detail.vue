  ...
  methods: {
    ...
    getDetailInfo() {
      getOrderDetail({ orderUuid: this.$route.params.id }).then(res => {
        ...
        if (data.typeTrip === 4) {
          // 有数据时
          if (data.routeTrafficDto && data.routeTrafficDto.trafficNumber) {
            if (this.showMap[data.mainStatus].indexOf("routeTrafficDto") === -1) {
              this.showMap[data.mainStatus].push("routeTrafficDto")
            }
          }
        }
        let orderTableMap =
        ...
        for (let key in data) {
          ...
          this.orderRightData.forEach(item => {
            ...
            // 判断是否展示接机信息
            if (data["routeTrafficDto"]) {
              item.value.forEach(cell => {
                if (cell.key == "trafficName") {
                  cell.value = data["routeTrafficDto"].trafficName
                }
                if (cell.key == "trafficNumber") {
                  cell.value = data["routeTrafficDto"].trafficNumber
                }
                if (cell.key == "status") {
                  // 航班状态：0计划;1起飞;2:延误;3到达;4取消
                  let map = {
                    0: "计划",
                    1: "起飞",
                    2: "延误",
                  ...
                  cell.value = map[data["routeTrafficDto"].status]
                }
                if (cell.key == "planDepartTime") {
                  cell.value = data["routeTrafficDto"].planDepartTime
                }
                if (cell.key == "planArrivalTime") {
                  cell.value = data["routeTrafficDto"].planArrivalTime
                }
                if (cell.key == "estimatedDepartTime") {
                  cell.value = data["routeTrafficDto"].estimatedDepartTime
                }
                if (cell.key == "estimatedArrivalTime") {
                  cell.value = data["routeTrafficDto"].estimatedArrivalTime
                }
                if (cell.key == "actualDepartTime") {
                  cell.value = data["routeTrafficDto"].actualDepartTime
                }
                if (cell.key == "actualArrivalTime") {
                  cell.value = data["routeTrafficDto"].actualArrivalTime
                }
              })
            } else {
              if (item.key == "routeTrafficDto") {
                item.isShow = true
              }
            }
          })
