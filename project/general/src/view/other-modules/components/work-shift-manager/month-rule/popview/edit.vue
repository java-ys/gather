    ...
    >
      <div>
        <FormItem label="休息天数" prop="day">
          <Select
            v-model="commonFormData.day"
            placeholder="请选择"
            style="width: 100px;"
          >
            <Option
              v-for="cell in restDayList"
              :value="cell.value"
              :key="cell.value"
              >{{ cell.label }}</Option
            >
        ...
        <FormItem label="日均在线时长" prop="onlineDuration">
          <InputNumber
            v-model="commonFormData.onlineDuration"
            :min="1"
            :max="1440"
            placeholder="请输入时长: 单位(分钟)"
            style="width:200px"
          ></InputNumber>
          分钟
        </FormItem>
        <FormItem label="日均高峰时长" prop="heightDuration">
          <InputNumber
            v-model="commonFormData.heightDuration"
            :min="1"
            :max="1440"
            placeholder="请输入时长: 单位(分钟)"
            style="width:200px"
    ...
    const testNumber = (rule, value, callback) => {
      if (this.commonFormData[rule.field] === null) {
        if (this.parentParams.examineType === "D") {
          if (rule.field === "onlineDuration")
            callback(new Error("请填写日在线时间"));
          if (rule.field === "heightDuration")
            callback(new Error("请填写日高峰时间"));
        } else {
          if (rule.field === "onlineDuration")
            callback(new Error("请填写日均高峰时间"));
          if (rule.field === "heightDuration")
            callback(new Error("请填写日均在线时间"));
        }
      } else callback();
    };
        ...
        heightDuration: [
          { required: true, validator: testNumber, triger: "change" },
          {
            validator: function(rule, params, callback) {
              if (
                _this.commonFormData.onlineDuration &&
