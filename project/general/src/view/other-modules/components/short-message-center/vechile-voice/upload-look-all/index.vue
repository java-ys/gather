  ...
  <div>
    <div style="display:flex" :style="{ width: width + 'px' }">
      ...
      <Button v-if="fileType === 2" type="success" @click="lookIt(2)"
        >播放视频</Button
      >
      <Button v-if="fileType === 3" type="success" @click="lookIt(3)"
        >播放音频</Button
      >
      <Upload
        v-if="isShowUpload"
      ...
      >
        <Button :loading="uploadLoading" type="primary">
          <span v-if="!uploadLoading">{{ btnUploadTile }}</span>
...
import lookAudio from "./lookAudio";
...
export default {
  ...
  data() {
    return {
      ...
      uploadLoading: false,
      maxSize: 0,
      btnUploadTile:this.uploadtitle
    };
  },
  watch: {
    defaultUrl() {
    ...
    uploadtitle(){
      this.btnUploadTile = this.uploadtitle
    }
  },
  mounted() {
  ...
  methods: {
    ...
    lookIt(res) {
      ...
      if (res === 3) {
        this.$refs.lookAudio.init(this.lookFileUrl);
      }
    }
  }
};
