<template>
  <div class="driver-configure">
    ...
    <div class="button-wrap">
      <Button type="primary" v-hasAuth="'driver-recruit-landing-config-add'" @click="addConfigure">新建招募落地页</Button>
    </div>
    <v-table ref="driverList" :current="tableConfig.current" :total="tableConfig.total" :pageSize="tableConfig.pageSize" :parColumns="tableConfig.tableColumns" :parTableData="tableConfig.tableData" @changePage="tablePageChange" @changePageSize="tablePageSizeChange">
    </v-table>
    <!-- 操作记录弹框 -->
...
import { getCityList, getRecruitList, recruitSetOff, recruitSetOn, recommendLog } from "_o/api/operator-carrier";

export default {
  name: "driver-recruit-landing-page",
  data() {
    return {
  ...
  methods: {
    ...
    addConfigure() {
      this.$router.push({
        path: "/opCenter/recomd-driver-manage/driver-recruit-landing-page/create"
      });
    },
    editConfigure(id) {
      this.$router.push({
        path:
          "/opCenter/recomd-driver-manage/driver-recruit-landing-page/edit/" + id
      });
    },
    translateCityList() {
      getCityList({}).then(res => {
        let list = cpTranslate(res.data.data);
        this.inputList[2].cascaderList = [
    ...
    finishData(data) {
      recruitSetOff({
        cityConfId: data.cityConfId
      }).then(res => {
        this.$Message.success("此配置已被停用");
        this.getTableConfigData();
      });
    },
    // 启用数据
