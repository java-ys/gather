<template>
    <div class="passenger-recommand-driver">
        ...
        <div class="button-wrap">
            <Button type="primary" @click="showAdd = true">新增推荐奖励配置</Button>
            <Button type="primary" @click="getAwardChange" style="float: right">奖励信息</Button>
        </div>
        <v-table :current="tableConfig.current" :total="tableConfig.total" :pageSize="tableConfig.pageSize"
            :parColumns="tableConfig.tableColumns" :parTableData="tableConfig.tableData" @changePage="tablePageChange"
            @changePageSize="tablePageSizeChange">
        </v-table>
        <!-- 操作记录弹框 -->
        <Modal v-model="showRecord" title="操作记录">
            <div class="showRecord-content">
                <v-table :current="modalTable.current" :pageSize="modalTable.pageSize"
                    :parColumns="modalTable.tableColumns" :parTableData="modalTable.tableData" :isShowPage="false">
                </v-table>
            </div>
            <div slot="footer">
    ...
    methods: {
        translateCityList() {
            getCityList({}).then(res => {
                ...
                this.inputList[2].cascaderList = [{ label: "全国所有城市", value: 1 }, ...list];
