<template>
  <div>
    ...
    <export-file exportUrl="/admin/driver/compliance/workOrderStatistics/export" btnText="批量导出" :queryData="searchParams"
      exportFileName="工单汇总统计.xls" style="margin-bottom: 20px;" v-hasAuth="'order-collect-export'"></export-file>
...
<script>
import { orderCollect } from './fields';
import exportFile from '_a/export-file/exportFile.vue';
import { mixins } from './fields';
import { getOrderCollectList, getAgentByCity, toResult } from '_o/api/quality-control.js';
import { getAgentList } from '_g/api/common.js';

const { inputList, columns } = orderCollect;
export default {
  name: 'orderCollect',
  ...
  methods: {
    ...
    async getTableData() {
      ...
      let [err, data] = await toResult(getOrderCollectList(conf));
      if (err) return;
