    ...
    >
      ...
      <div v-if="draggable">
        ...
        <div slot="tip" class="el-upload__tip">
          只能上传{{this.type || "excel"}}文件{{this.type === 'excel' ? '，最多导入20000条数据' : ''}}
        </div>
      </div>
      <el-dialog
...
<script>
import { uploadImgss } from "_o/api/salary/interview";
import { getToken } from "@/libs/util"
import { checkAndHandleCompression, convertBase64UrlToFile, TOKEN_KEY } from "_o/common/common";
...
export default {
  ...
  methods: {
    ...
    getRequest(formData, params) {
      ...
      return this.interface(formData).then(res => {
