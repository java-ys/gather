 ...
 * @LastEditors: wangp
 * @LastEditTime: 2020-07-16 20:35:57
-->
<template>
  <div>
    <el-form ref="editForm" :model="formData" class="new-demo-ruleForm">
      <el-card style="margin-top: 10px">
        ...
        <template v-if="!showForm">
          <span class="check-text">(勾选后，日流水达标固定奖励生效)</span><br />
        </template>
        <template v-else>
          <el-card
...
export default {
  props: ["floatWaterAwardData", "isEdit", "beginTime"],
  data() {
    return {
      validateIntAmount,
      limitCheck: "",
  ...
  methods: {
      ...
      if (this.showForm) {
        ...
        this.$refs.editForm.validate(valid => {
          if (valid) {
            ...
            if (!num) {
              localStorage.setItem("floatWaterData", JSON.stringify(this.formData));
              flag = true;
            } else {
              this.$alert("生效日期不能为空");
            }
    ...
    setDataTimeList() {
      if (this.isEdit && this.floatWaterAwardData.length) {
        this.formData.ruleList.forEach(item => {
    ...
    checkStatus() {
      if (this.isEdit && this.floatWaterAwardData.length) {
        this.limitCheck = "1";
        this.formData.ruleList = JSON.parse(JSON.stringify(this.floatWaterAwardData));
      }
    },
    setDates(val, isEdit) {
      this.formData.ruleList.forEach(item => {
    ...
    handleChange(val) {
      if (this.beginTime) {
        if (val == "1") {
          this.$emit("handleChoose", "floatWaterAward");
        } else {
          this.showForm = false;
        }
      } else {
