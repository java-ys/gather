<template>
  <div>
    ...
    <el-dialog :visible.sync="photoing" append-to-body class="dialog-size" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="text-align: center; margin-top: 10px">
        <div id="photo-text" style="font-size: 18px;">正在生成并上传照片，请稍等片刻...</div>
      </div>
    </el-dialog>
  </div>
...
<script>
import { photoParamSet, photoParamGet, photoGrabImage, uploadPhoto } from "_o/api/salary/profile";
...
export default {
  ...
  data() {
    return {
      photoing: false,
      photoErrorMsg: "拍摄失败，可能原因： 1.未安装高拍仪程序 2.未连接高拍仪设备 3.未开启高拍仪服务 4.服务状态异常"
    }
  },
  methods: {
    // 拍照参数初始化 ：像素、镜头
    photoInit() {
      ...
      that.photoing = true;
      // camidx 0 文档头 1 人像头
      photoParamSet({ "camidx": this.camera }).then(res => {
        if (res.data.code != "0") {
    ...
    startPhoto() {
      let that = this;
      // camidx 0 文档头 1 人像头
      photoParamGet({ "type": "resolution", "camidx": this.camera }).then(res => {
        if (res.data.code != "0") {
    ...
    makePhoto() {
      let that = this;
      // camidx 0 文档头 1 人像头
      photoGrabImage({ "filepath": "base64", "rotate": "0", "camidx": this.camera, "cutpage": "1" }).then(res => {
        if (res.data.code != "0") {
          that.$alert(this.photoErrorMsg);
          that.photoing = false;
        } else {
          let photoBase64 = res.data.photoBase64;
          let file = convertBase64UrlToFile("data:image/jpeg;base64," + photoBase64, "photoImage" + new Date().getTime());
