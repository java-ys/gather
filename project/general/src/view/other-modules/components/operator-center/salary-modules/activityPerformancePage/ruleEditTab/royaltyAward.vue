 ...
 * @LastEditors: wangp
 * @LastEditTime: 2020-07-10 11:39:11
-->
<template>
  <div>
    <el-form ref="editForm" :model="formData" class="new-demo-ruleForm">
      <el-card style="margin-top: 10px">
        ...
        <template v-if="!showForm">
          <span class="check-text">(勾选后，高峰时段流水额外抽成奖励生效)</span><br />
        </template>
        <template v-else>
          <el-card
                ...
                <el-row v-for="(it, idx) in item.cfgList" :key="idx">
                  ...
                  <el-col :span="3">
                    <el-button
                      ...
                      size="small"
                      icon="el-icon-minus"
                      @click="handleRemoveRule(item, idx)"
                    >
                    </el-button>
                    <el-button
                      type="primary"
...
<script>
import { royaltyListData } from "./fields";
import {
  validateAmount,
  validatePercent,
  isEmptyObject,
...
export default {
  props: ["royaltyAwardData", "isEdit", "beginTime"],
  data() {
    return {
      validateIntAmount,
      validatePercent,
      ...
      showForm: false,
      formData: {
        ruleList: JSON.parse(JSON.stringify(royaltyListData))
      },
      dateList: [],
      pickList: []
    };
  ...
  methods: {
      ...
      if (this.showForm) {
        ...
        this.$refs.editForm.validate(valid => {
          if (valid) {
            ...
            if (!num) {
              localStorage.setItem("royaltyAwardData", JSON.stringify(this.formData));
              flag = true;
            } else {
              this.$alert("生效日期不能为空");
            }
    ...
    setDataTimeList() {
      if (this.isEdit && this.royaltyAwardData.length) {
        this.formData.ruleList.forEach(item => {
    ...
    checkStatus() {
      if (this.isEdit && this.royaltyAwardData.length) {
        this.limitCheck = "1";
        // this.showForm = true;
        this.formData.ruleList = JSON.parse(JSON.stringify(this.royaltyAwardData));
      }
    },
    setDates(val, isEdit) {
      this.formData.ruleList.forEach(item => {
    ...
    handlePlus(val) {
      if (this.formData.ruleList.length < 7) {
        if (val == "add") {
          this.formData.ruleList.push(JSON.parse(JSON.stringify(royaltyListData[0])));
        } else {
          let i; let
            m;
    ...
    handleAddRule(item, idx) {
      if (item.cfgList.length === 4) return this.$message({ message: "无法新增，最多创建4条！", type: "warning" });
      const field = {
        periodStart: "",
        periodEnd: "",
        rewardValue: "",
      ...
      // item.cfgList.push(field)
      item.cfgList.splice(idx + 1, 0, field)
    },
    handleRemoveRule(item, idx) {
      if (item.cfgList.length === 1) return this.$message({ message: "无法删除，最少保留1条！", type: "warning" });
      this.$delete(item.cfgList, idx)
    },
    handleChange(val) {
      if (this.beginTime) {
        if (val == "1") {
          this.$emit("handleChoose", "royaltyAward");
        } else {
          this.showForm = false;
        }
      } else {
