...
<template>
  ...
  <div>
      ...
      <el-form :inline="true" :model="queryForm" class="demo-form-inline" size="mini" label-position="right">
        ...
        <el-row>
          <el-form-item style="float: right">
            <el-button v-if="hasPermission('weekRewardReview-manage-query')" v-preventReClick="3000" type="primary" @click="paramSelect">搜索</el-button>
            <el-button v-if="hasPermission('weekRewardReview-manage-query')" v-preventReClick="3000" type="info" @click="reset">重置</el-button>
          </el-form-item>
        </el-row>
      </el-form>
    </div>
    ...
    <div class="main buttons">
      <export-button :loading="exportLoading" queryExport="weekRewardReview-manage-export" :queryForm="queryForm" @fatherExport="exportExcel" @loadingStatus="loadingStatus"></export-button>
    </div>
    <!-- 周收入应发审核-列表项 -->
    <div style="tb-mt">
      <el-table v-loading="listLoading" element-loading-text="正在加载" :row-style="{ backgroundColor: 'transparent' }" :data="tableData" size="mini" :header-cell-style="{ background: '#F5F4F2' }" fit highlight-current-row>
        ...
        <el-table-column prop="totalAmount" label="周收入应发总金额" min-width="120"></el-table-column>
        <el-table-column prop="driverCount" label="总人数" min-width="120"></el-table-column>
        <el-table-column prop="apieceAmount" label="人均应发金额" min-width="120"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="160" class-name="td_bgf">
          <template slot-scope="scope">
            <el-button v-if="hasPermission('weekRewardReview-manage-examine') && scope.row.auditType == 0" size="small" type="text" @click="setApproval(scope.row)">审核</el-button>
            <el-button v-if="hasPermission('weekRewardReview-manage-detail')" size="small" type="text" @click="getDetail(scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    exportExcel() {
      ...
      if (Object.keys(requestParams).length === 0) {
        ...
        weekRewardExport(requestParams).then(res => {
          excelDownload(res.data, "周收入应发审核数据信息");
        });
      }
    },
    // 列表水印
