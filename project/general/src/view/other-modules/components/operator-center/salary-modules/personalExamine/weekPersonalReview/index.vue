...
<template>
  ...
  <div>
      ...
      <el-form :inline="true" :model="queryForm" class="demo-form-inline" size="mini" label-position="right">
        <el-row>
          <el-col :span="6">
            <el-form-item label="奖励周" label-width="100px">
              <el-date-picker v-model="dateArr" class="text-width" :clearable="false" type="week" placeholder="请选择" :format="'yyyy 第 WW 周'" :picker-options="pickerOptions" @change="TimeDescFormatter">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item label="运营模式" label-width="100px">
              <el-select v-model="queryForm.salaryType" placeholder="请选择运营模式" class="text-width">
        ...
        <el-row>
          <el-form-item style="float: right">
            <el-button v-if="hasPermission('weekPersonalReview-manage-query')" v-preventReClick="3000" type="primary" @click="paramSelect">搜索</el-button>
            <el-button v-if="hasPermission('weekPersonalReview-manage-query')" v-preventReClick="3000" type="info" @click="reset">重置</el-button>
          </el-form-item>
        </el-row>
      </el-form>
    </div>
    ...
    <div class="main buttons">
      <export-button :loading="exportLoading" queryExport="weekPersonalReview-manage-export" :queryForm="queryForm" @fatherExport="exportExcel" @loadingStatus="loadingStatus"></export-button>
    </div>
    <!-- 周收入实发审核-列表项 -->
    <div style="tb-mt">
      <el-table v-loading="listLoading" element-loading-text="正在加载" :row-style="{ backgroundColor: 'transparent' }" :data="tableData" size="mini" :header-cell-style="{ background: '#F5F4F2' }" fit highlight-current-row>
        <el-table-column label="奖励周" min-width="180">
          <template slot-scope="scope">
            <span>{{scope.row.beginTime ? scope.row.beginTime.substr(0, 10) : ""}}&nbsp;~&nbsp;{{scope.row.endTime ? scope.row.endTime.substr(0, 10) : ""}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="cityName" label="城市" min-width="120"></el-table-column>
        <el-table-column prop="auditType" label="状态" :formatter="auditTypeFormatter" min-width="120"></el-table-column>
        <el-table-column prop="salaryType" label="运营模式" :formatter="salaryTypeFormatter" min-width="120"></el-table-column>
        <el-table-column prop="totalPersonalIncomeTax" label="个税总金额" min-width="120"></el-table-column>
        <el-table-column prop="totalSocialSecurityFee" label="社保扣款合计" min-width="120"></el-table-column>
        <el-table-column prop="totalAmountAfterTax" label="周收入实发总金额" min-width="120"></el-table-column>
        <el-table-column prop="amountLaborService" label="周收入实发总金额(劳务发放)" min-width="180"></el-table-column>
        <el-table-column prop="driverCount" label="总人数" min-width="120"></el-table-column>
        <el-table-column prop="actualPaidNum" label="实际发放人数" min-width="120"></el-table-column>
        <el-table-column prop="apieceActualAmount" label="人均实发金额" min-width="120"></el-table-column>
        <el-table-column prop="apieceLaborActualAmount" label="人均实发金额(劳务发放)" min-width="160"></el-table-column>
        <el-table-column prop="applicantName" label="提交人" min-width="120"></el-table-column>
        ...
        <el-table-column prop="auditorName" label="审核人" min-width="120"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="160" class-name="td_bgf">
          <template slot-scope="scope">
            <el-button v-if="hasPermission('weekPersonalReview-manage-examine') && scope.row.auditType == 0" size="small" type="text" @click="setApproval(scope.row)">审核</el-button>
            <el-button v-if="hasPermission('weekPersonalReview-manage-detail')" size="small" type="text" @click="getDetail(scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
      ...
      handler(val) {
        if (val) {
          this.queryForm.beginTime =
            parseTime(
              new Date(this.dateArr.getTime() - 1000 * 60 * 60 * 24),
              "{y}-{m}-{d}"
            ) + " 00:00:00";
          this.queryForm.endTime =
            parseTime(
              new Date(this.dateArr.getTime() + 1000 * 60 * 60 * 24 * 5),
              "{y}-{m}-{d}"
            ) + " 23:59:59";
        }
      },
  ...
  methods: {
    ...
    getDetail(row) {
      let data = this.dislodgeZero(row.beginTime).split(" ");
      this.$router.push({
        name: row.salaryType == 1 ? "weekReward/modeA" : row.accountingType == 1 ? "weekReward/modeUP" : "weekReward/driverStream",
        params: {
          cityCode: row.cityCode,
          cityName: row.cityName,
          year: new Date(row.beginTime).getFullYear(),
          firstDayOfWeek: data[0],
          week: new Date(row.beginTime)
        }
      });
    },
    setApproval(row) {
    ...
    exportExcel() {
      ...
      if (Object.keys(requestParams).length === 0) {
        ...
        weekRewardExport(requestParams).then(res => {
          excelDownload(res.data, "周收入实发审核数据信息");
        });
      }
    },
    // 列表水印
    ...
    auditTypeFormatter(row) {
      if (row.auditType == 0 && row.oaAudit == 0) {
        return "待审核";
      } else if (row.auditType == 0 && row.oaAudit == 1) {
        return "待审核(OA)";
