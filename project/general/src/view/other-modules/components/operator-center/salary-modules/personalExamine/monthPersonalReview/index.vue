...
<template>
  ...
  <div>
      ...
      <el-form :inline="true" :model="queryForm" class="demo-form-inline" size="mini" label-position="right">
        ...
        <el-row>
          <el-form-item style="float: right">
            <el-button v-if="hasPermission('monthPersonalReview-manage-query')" v-preventReClick="3000" type="primary" @click="paramSelect">搜索</el-button>
            <el-button v-if="hasPermission('monthPersonalReview-manage-query')" v-preventReClick="3000" type="info" @click="reset">重置</el-button>
          </el-form-item>
        </el-row>
      </el-form>
    </div>
    ...
    <div class="main buttons">
      <export-button :loading="exportLoading" queryExport="monthPersonalReview-manage-export" :queryForm="queryForm" @fatherExport="exportExcel" @loadingStatus="loadingStatus"></export-button>
    </div>
    <!-- 月工资实发审核-列表项 -->
    <div style="tb-mt">
      <el-table v-loading="listLoading" element-loading-text="正在加载" :row-style="{ backgroundColor: 'transparent' }" :data="tableData" size="mini" :header-cell-style="{ background: '#F5F4F2' }" fit highlight-current-row>
        <el-table-column label="薪酬月份" min-width="90">
        ...
        <el-table-column prop="auditType" label="状态" :formatter="auditTypeFormatter" min-width="110"></el-table-column>
        ...
        <el-table-column prop="totalSocialSecurityFee" label="社保总金额" min-width="120"></el-table-column>
        <el-table-column prop="totalAmountAfterTax" label="月工资实发总金额" min-width="120"></el-table-column>
        <el-table-column prop="amountLaborService" label="月工资实发总金额(劳务发放)" min-width="120"></el-table-column>
        <el-table-column prop="driverCount" label="总人数" min-width="110"></el-table-column>
        <el-table-column prop="actualPaidNum" label="实际发放人数" min-width="110"></el-table-column>
        <el-table-column prop="apieceActualAmount" label="人均实发金额" min-width="110"></el-table-column>
        <el-table-column prop="apieceLaborActualAmount" label="人均实发金额(劳务发放)" min-width="110"></el-table-column>
        <el-table-column prop="applicantName" label="提交人" min-width="110"></el-table-column>
        <el-table-column prop="applyTime" label="提交审核时间" min-width="140"></el-table-column>
        <el-table-column prop="batchTypes" label="审核类型" min-width="120">
          <template slot-scope="scope">
            {{"月工资实发"}}
          </template>
        </el-table-column>
        <el-table-column prop="auditorName" label="审核人" min-width="110"></el-table-column>
        <el-table-column prop="auditPassedTime" label="审核通过时间" min-width="140"></el-table-column>
        <el-table-column prop="remark" label="备注" min-width="110"></el-table-column>
        <el-table-column label="操作" fixed="right" min-width="200" class-name="td_bgf">
          <template slot-scope="scope">
            <el-button v-if="hasPermission('monthPersonalReview-manage-examine') && scope.row.auditType == 0" size="small" type="text" @click="setApproval(scope.row)">审核</el-button>
            <el-button v-if="hasPermission('monthPersonalReview-manage-detail')" size="small" type="text" @click="getDetail(scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      if (Object.keys(requestParams).length === 0) {
        ...
        weekRewardExport(this.queryForm).then(res => {
          excelDownload(res.data, "月工资实发审核数据信息");
        });
      }
    },
    // 列表水印
    ...
    getData(params) {
      ...
      if (this.auditDateArr && this.auditDateArr.length > 0) {
        this.queryForm.auditCreatedTimeStart =
          this.auditDateArr[0] + " 00:00:00";
        let date = new Date(this.auditDateArr[1]);
        date.setDate(date.getDate());
        this.queryForm.auditCreatedTimeEnd =
          dateTimeFormat(date.toDateString(), "yyyy-MM-dd") + " 23:59:59";
      } else {
        this.queryForm.auditCreatedTimeStart = "";
        this.queryForm.auditCreatedTimeEnd = "";
      }
      if (this.examineDateArr && this.examineDateArr.length > 0) {
        this.queryForm.auditPassedTimeStart =
          this.examineDateArr[0] + " 00:00:00";
        let date = new Date(this.examineDateArr[1]);
        date.setDate(date.getDate());
        this.queryForm.auditPassedTimeEnd =
          dateTimeFormat(date.toDateString(), "yyyy-MM-dd") + " 23:59:59";
      } else {
        this.queryForm.auditPassedTimeStart = "";
        this.queryForm.auditPassedTimeEnd = "";
      }
      // this.queryForm.batchTypes = [6];
      let requestParams = JSON.parse(JSON.stringify(this.queryForm));
      for (let propName in requestParams) {
        if (
          requestParams.hasOwnProperty(propName) &&
