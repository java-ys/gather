    ...
    >
      <el-form ref="form" :model="form" label-width="80px">
        <el-form ref="formTime" :inline="true" :model="form">
          ...
          <el-form-item label="奖励月" style="margin-left: 12px">
            <div style="width: 202px">{{form.moneyDate}}</div>
          </el-form-item>
        </el-form>
        <el-form ref="formChild" :inline="true" :model="form">
          <div v-for="(item, index) in form.list">
            <el-form-item label="奖励项目" style="margin-left: 12px" :prop="'list.' + index + '.moneyItem'" :rules="[{ required: true, message: '必填字段', trigger: 'blur' }]">
              <el-input v-model="item.moneyItem" maxlength="30"></el-input>
            </el-form-item>
            <el-form-item label="奖励金额" style="margin-left: 12px" :prop="'list.' + index + '.money'" :rules="[{ required: true, message: '必填字段', trigger: 'blur' },{ validator: validNumber, trigger: 'blur' }]">
        ...
        <el-form-item label="备注" style="display: block;">
          <el-input v-model="form.remark" maxlength="120" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }"></el-input>
        </el-form-item>
      </el-form>
    ...
    init(row) {
      queryDeductMoney({
        ...
        if (data.length && data[0].updateTime) {
          date = parseTime(new Date(data[0].updateTime).getTime(), "{y}-{m}");
        } else {
          date = parseTime(new Date().getTime(), "{y}-{m}")
        }
        ...
        if (valid) {
          ...
          this.form.list.forEach(element => {
            arr.push({
              ...
              moneyDate: parseTime(new Date(this.form.moneyDate).getTime(), "{y}-{m}"),
              remark: this.form.remark,
              driverUuid: this.addBonusRow.driverUuid,
              salaryUuid: this.addBonusRow.uuid,
              moneyType: "1",
              id: element.id,
            })
          });
