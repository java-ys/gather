        ...
        <template v-if="itemTab.answerList.length">
          <div style="margin-right:16px">
            <div class="loadBar">
              ...
              ></DatePicker>
              <Button v-hasAuth="'nps_send-exportAnswerDetail'" type="primary" :disabled="!canExport"
              ...
              <Button type="primary" :disabled="!canExport" @click="exportPDF(indexTab)">
                导出答题详细pdf
              </Button>
              <Button type="success" :disabled="canExport" @click="downloadAnswerExcel(true, itemTab)">下载</Button>
            </div>
            <div class="loadBar">
              <Button v-hasAuth="'nps_send-answerSummary'" type="info"
          ...
          <div ref="analyse">
            <div v-for="(item, index) in itemTab.answerList" :key="index" class="ques-item">
              ...
              <Button
                ...
                v-hasAuth="'nps_send-blankAnswerSummary'"
                class="margin8"
                type="info"
...
export default {
  ...
  methods: {
    ...
    exportPDF(index) {
      html2pdf(this.$refs.analyse[index], {
        margin: 10,
        filename: "答题详细.pdf",
    ...
    downloadAnswerExcel(downloadFlag, itemTab, type) {
      ...
      exportAnswerDetail(params).then((res) => {
        if (res.data.success) {
          if (downloadFlag) {
            this.canExport = true
            window.location.href = res.data.data.httpsUrl
          }
        } else {
          this.canExport = true
        }
