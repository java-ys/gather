<template>
  <div>
    ...
    >
      <FormItem :label-width="0">
        <Button
          v-hasAuth="'nps_send-silentQuery'"
          type="default"
          style="width:120px"
          @click="silenceSetting"
        >
          沉默逻辑设置
        </Button>
      </FormItem>
    ...
    >
      ...
      <div slot="footer">
        ...
            <Button v-hasAuth="'nps_send-silentEdit'" type="primary" size="large" @click="confirmSilence">确定</Button>
      </div>
    </Modal>
    <log
      ref="log"
...
import { getSends, stopUse, silentQuery, silentEdit } from "_o/api/nps/app.js";
...
export default {
  ...
  methods: {
    ...
    // 新建
    newTask(val) {
      this.$router.push({ name: "nps-send-add" });
    },
    ...
    handleStopUse(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          stopUse({ configId: row.configId }).then(res => {
            if (res.data.success) {
              this.getTableList();
            }
          });
