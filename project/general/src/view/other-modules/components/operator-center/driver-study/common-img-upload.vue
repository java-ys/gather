...
<template>
  <div>
    <div class="up-out" :style="{height:sheep.height}">
      <div v-if="defaultFile || defaultImg">
        <!-- 文件 -->
        <div v-if="defaultFile" class="demo-upload-file">
          <span v-if="defaultFileName">{{defaultFileName}}</span>
...
<script>
// 限制宽高，大小，格式，单图片上传组件
import { getDownloadToken } from "_o/api/file";
    ...
    beforeUpload(file) {
      return new Promise(async (reslove, reject) => {
        ...
        if(this.maxFileSize) {
          ...
          isAllow = !(fileMb > this.maxFileSize);
          msg = `请上传大小在 ${this.maxFileSize}MB 以内的文件`;
          if (!isAllow) {
            that.$Message.error(msg);
            return reject(false);
          }
        ...
        const [err, data] = await toResult(getDownloadToken({}));

        if (err) {
          // that.$Message.error("获取上传图片token失败");
          return reject(false);
        }
        this.headers.token = data.msg;
...
.demo-upload-file:hover .demo-upload-list-cover {
  display: block;
}
