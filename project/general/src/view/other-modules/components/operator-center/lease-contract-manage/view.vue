  ...
  <div>
    ...
    >
      <div class="view-wrapper">
        <Row class="view-row">
          <Col span="3">合同名称：</Col>
          <Col span="9">{{ contractName }}</Col>
          <Col span="3">分类：</Col>
          <Col span="9">{{ customizationFlag | customization }}</Col>
        </Row>
        <Row class="view-row">
          <Col>适用对象:</Col>
          <p v-if="customizationFlag === 0">
            所有CP公司（已配置定制版的CP公司除外）
          </p>
          <p v-else>{{ cpNames }}</p>
        </Row>
        <Row class="view-row">
...
<script>
import { queryContractDetail } from "_o/api/lease";
    ...
    getContractDetail() {
      ...
      queryContractDetail(params).then(async res => {
        if (res && res.data && res.data.success) {
          ...
          this.endTime = this.$moment(data.endTime).format(
            "YYYY-MM-DD HH:mm:ss"
          );
