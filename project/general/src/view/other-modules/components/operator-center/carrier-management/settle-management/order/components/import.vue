          ...
          <div class="down-container">
            ...
            <div class="c1">
              ...
              <div class="border-dashed d-b" style="margin-right: 6px">
                <ExportFile
                  exportUrl="/download/结算账单模板.xlsx"
                  exportFileName="运营商考核结算账单模板.xlsx"
                  :queryData="{}"
                  btnText="下载模板"
                  ajaxMethod="get"
                ></ExportFile>
...
import { batchImportService } from "_o/api/settle";
  ...
  methods: {
    ...
    addSubmit() {
      this.$refs["_form"].validate(async (valid) => {
        ...
        const [err, data] = await this.toResult(batchImportService(p));
        ...
        if (data) {
          ...
          this.$Modal.success({
            title: "导入数据",
            content: `上传成功: ${data?.successDtoList.length}条`,
            onOk: () => {
              this.close(true);
            },
          });
