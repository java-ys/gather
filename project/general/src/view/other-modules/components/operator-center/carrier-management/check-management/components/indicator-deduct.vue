<template>
  <div class="mt-20">
    <Form v-model="it" :label-width="150">
      <FormItem label="扣减事项：" prop="">
        <span>{{ it.name || it.indexName }}</span>
      </FormItem>
      <FormItem label="扣减说明：" prop="remark">
        <Row>
          <Input
            type="textarea"
            v-model="it.remark"
...
export default {
  ...
  methods: {
    ...
    getData() {
      let data = deepClone({}, this.it)
      try {
        data.indexRuleJson = JSON.stringify(data.indexRuleJson)
      }catch {
        data.indexRuleJson = ''
      }
      return data;
