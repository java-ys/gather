<template>
  <div class="check-management-check">
    ...
    <div class="button-wrap">
      <Button v-hasAuth="'check-goal-import'" type="primary" @click="importFile">批量导入</Button>
      <export-file
        v-hasAuth="'check-goal-export'"
        type="success"
        :isDisabled="tableData.length===0"
        exportUrl="/api/operate/assess/target/exportAgentAssessTarget"
        exportFileName="考核目标管理数据.xlsx"
        :queryData="searchData"
        btnText="批量导出"
      ></export-file>
      <Button v-hasAuth="'check-goal-add'" type="primary" @click="addNew" :loading="false">新建</Button>
    </div>
    <m2-table
      class="top15"
      :current="current"
...
import {agentTargetService, deleteAgentTargetService} from "_o/api/settle/goal.js"
...
export default {
  ...
  data() {
    return {
      ...
      detailData: {},
      fileVisible: false,  // 导入文件弹层
      cityList: [],
      indicatorList:[], // 指标列表
    };
  },
  created() {
    this.func = agentTargetService;
    this.getList();
  },
  mounted() {
    this.getCityList();
  ...
  methods: {
    ...
    deleteRow({uuid, indexName, assessTime, agentName}) {
      let self = this;
      this.$Modal.warning({
        title: "提示",
        closable: true,
        content: `确认删除【${agentName}】【${assessTime}】考核中【${indexName}】目标值？如果考核已经下发到运营商，删除后数据无法回溯。`,
        onOk: async function () {
          const [err] = await self.toResult(deleteAgentTargetService({ uuid }));
          if (err) {
            return;
          }
      ...
      if (selection && selection.length === 0) {
        ...
        data.forEach((item) => {
          if (this.selectedList.has(item.id))
            this.selectedList.delItem(item.id);
        });
      } else {
