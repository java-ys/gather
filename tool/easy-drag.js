...
const formatVector = (vector, range) => {
    let x = vector[0];
    let y = vector[1];
    x = Math.max(x, range[2]); // 左
    x = Math.min(x, range[3]); // 右
    y = Math.max(y, range[0]); // 上
    y = Math.min(y, range[1]); // 下
    return [x, y];
};
...
const enableDrag = (element, options) => {
    ...
    const onMouseDown = (e) => {
        ...
        if (outerElement && element && innerElement) {
            ...
            console.log(outerElementRect, 'outerElementRect')
            console.log(elementRect, 'elementRect')
            console.log(draggingMoveVectorRange, 'draggingMoveVectorRange')
        }
